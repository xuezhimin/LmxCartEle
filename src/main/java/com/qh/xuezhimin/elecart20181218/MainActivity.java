...
public class MainActivity extends AppCompatActivity implements DataCall<List<Data>> {
    ...
    private void calculatePrice(List<Data> dataList) {
        double totalPrice = 0;
        int totalNum = 0;
        for (int i = 0; i < dataList.size(); i++) {//循环的商家
            Data data = dataList.get(i);
            for (int j = 0; j < data.getSpus().size(); j++) {
                Spus spus = data.getSpus().get(j);
                //计算价格
//                totalPrice = totalPrice + (spus.getPraise_num() * spus.getSkus().get(0).getPrice());
                totalNum += data.getSpus().get(0).getPraise_num();//计数
            }
        }
